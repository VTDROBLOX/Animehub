...
-- ========================================
-- AUTO EGG EVENT - TICH HOP TAYHUB
-- ========================================
...
-- Ham nhat trung - fix loi cf la CFrame
local function CollectEgg()
    ...
    -- Tim Remote (khong dung WaitForChild de tranh yield)
    local EggRemote = nil
    pcall(function()
        EggRemote = game:GetService("ReplicatedStorage")
            .Modules.Net["RE/EasterServiceRE"]
    end)
    if not EggRemote then return false end

    local eggs = game:GetService("CollectionService"):GetTagged("EasterEgg26")
    ...
    for _, egg in pairs(eggs) do
        ...
        if uid and egg:IsA("BasePart") then
            ...
            -- BAM TRUNG 10 LAN - theo duoi trung chay
            for i = 1, 10 do
                if not egg or not egg.Parent then break end
                pcall(function()
                    -- Dung .Position (BasePart) chu khong dung CFrame attribute
                    root.CFrame = CFrame.new(egg.Position + Vector3.new(0, 2, 0))
                end)
                task.wait(0.1)
            end
            ...
            for i = 1, 5 do
                pcall(function()
                    EggRemote:FireServer("CollectEgg", {
                        UID = uid,
                        Additional = {}
                    })
                end)
...
-- Bay muot den dao - dung _tp chinh cua TayHub
local function FlyToIsland(dest)
    -- Tam dung farm tranh xung dot
    local sv = {
        StartFarm = _G.StartFarm,
        Level = _G.Level,
        Bone = _G.AutoFarm_Bone,
        Cake = _G.AutoFarm_Cake,
    }
    _G.StartFarm = false
    _G.Level = false
    _G.AutoFarm_Bone = false
    ...
    -- Bay muot bang _tp (chinh xac giong farm xuong/level)
    pcall(function()
        _tp(dest * CFrame.new(0, 8, 0))
    end)

    -- Cho bay xong theo khoang cach thuc te
    local root = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
    local dist = root and (dest.Position - root.Position).Magnitude or 500
    task.wait(math.clamp(dist / 250, 2, 12))

    -- Bat lai farm
    _G.StartFarm = sv.StartFarm
    _G.Level = sv.Level
    _G.AutoFarm_Bone = sv.Bone
    _G.AutoFarm_Cake = sv.Cake
end
...
-- UI - Dung Event tab cua TayHub (khong phai Main)
Event:AddSection("🥚 Auto Egg Event")
Event:AddDropdown({
    Name = "Chế Độ Nhặt Trứng",
    Options = {"Island", "Normal"},
    Default = "Island",
    Callback = function(v)
        _G.ChooseEgg = v
    end
...
Event:AddToggle({
    ...
    Description = "Bám trứng + bay mượt qua đảo như farm xương",
    Default = false,
    Callback = function(v)
        _G.AutoCollectEgg = v
