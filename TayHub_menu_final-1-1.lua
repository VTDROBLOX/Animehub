...
-- COMPAT Delta/Fluxus
if not hookfunction then hookfunction = function() end end
...
task.spawn(function()
    task.wait(3) -- cho script chinh load xong
    pcall(function()
...
local v2 = Window:MakeTab({
    Title = "Farming",
    Icon = "rbxassetid://91222518412026"
})
...
v0:AddProfile({
    Name = "Tay Hub",             
    Bio = "Tay Hub - Blox Fruit Script", 
    Avatar = "rbxassetid://91222518412026", 
    Cover = "rbxassetid://78825251117353", 
    Verified = true 
})
v0:AddSingleDiscordCard({
"Tay Hub",
        "Tham gia Discord ho tro!",
        "rbxassetid://91222518412026",
        "rbxassetid://78825251117353",
        1000,
        27000,
        "https://discord.gg/75HaB35ymY"
})

v0:AddParagraph({
    "Thông báo",
    "•TÀY HUB• SẼ SỚM RA BẢN CHÍNH THỨC"
})
...
v2:AddToggle({
    Name = "Kill all the monsters on the map ⚠️",
    Default = false,
    Callback = function(enabled)
        _G.BringMobFarm = enabled
        _G.FastV2 = true
        _G.FarmState = true
        _G.KilledCount = _G.KilledCount or 0

        if enabled then
            local p = game.Players.LocalPlayer
            local RS = game:GetService("ReplicatedStorage")

            -- Lay Remote dung format goc
            ...
            pcall(function()
                if p.PlayerGui:FindFirstChild("FarmCounter") then
                    p.PlayerGui.FarmCounter:Destroy()
                end
                ...
                -- Dem kill qua Notifications
                pcall(function()
                    p.PlayerGui.Notifications.ChildAdded:Connect(function(child)
                        task.wait(0.1)
                        if child:IsA("TextLabel") and (
                            child.Text:find("Exp") or
                            child.Text:find("EXP") or
                            child.Text:find("Da nhan") or
                            child.Text:find("Earned")
                        ) then
                            if _G.BringMobFarm then
                                _G.KilledCount = (_G.KilledCount or 0) + 1
                                pcall(function()
            ...
            -- Tu dong cam Fruit (CHINH XAC tu file goc)
            ...
            -- Logic farm: lap qua TUNG quai, dich chuyen den tung cai
            -- Giong code goc: PivotTo + RegisterHit + LeftClickRemote (M1)
            task.spawn(function()
                while _G.BringMobFarm do
                    task.wait(0.1)
                        ...
                        if root then
                            for _, target in pairs(workspace.Enemies:GetChildren()) do
                                ...
                                if hum and hum.Health > 0 and tRoot then
                                    ...
                                    -- Dich chuyen den quai (offset 2 studs phia tren)
                                    char:PivotTo(tRoot.CFrame * CFrame.new(0, 2, 0))
                                    -- Fast Attack + M1 (LeftClickRemote)
                                    if _G.FastV2 then
                                        ...
                                        if fruit and fruit:FindFirstChild("LeftClickRemote") then
                                            fruit.LeftClickRemote:FireServer(
                                                (tRoot.Position - root.Position).Unit,
                                                1
                                            )
                                        end
                                    end
            ...
            task.spawn(function()
                while _G.BringMobFarm do
                    ...
                    if root2 and not root2:FindFirstChild("VelocityHold") then
            ...
            -- Tat: xoa GUI + reset
            pcall(function()
                local gui = game.Players.LocalPlayer.PlayerGui:FindFirstChild("FarmCounter")
...
-- Bộ đếm ngược 7 ngày - TO, DE NHIN
local _bossRelease = os.time() + (7 * 24 * 3600)
local _bossCountPara = v2:AddParagraph({
...
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            local r = math.max(0, _bossRelease - os.time())
            local d = math.floor(r / 86400)
            local h = math.floor((r % 86400) / 3600)
            local m = math.floor((r % 3600) / 60)
            local s = r % 60
            _bossCountPara:SetDesc(string.format(
                "🇻🇳  Chức năng sẽ ra mắt trong bản chính thức\n\n" ..
                "🕐  %d ngày  %02d:%02d:%02d\n\n" ..
                "🇬🇧  Feature coming in official version\n\n" ..
                "🕐  %d days  %02d:%02d:%02d",
                d, h, m, s,
                d, h, m, s
            ))
...
-- Bộ đếm ngược 7 ngày - TO, DE NHIN
local _seaRelease = os.time() + (7 * 24 * 3600)
local _seaCountPara = v6:AddParagraph({
    Title = "⏳  Sắp Ra Mắt  /  Coming Soon",
    Desc = "Đang tính..."
})
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            local r = math.max(0, _seaRelease - os.time())
            local d = math.floor(r / 86400)
            local h = math.floor((r % 86400) / 3600)
            local m = math.floor((r % 3600) / 60)
            local s = r % 60
            _seaCountPara:SetDesc(string.format(
                "🇻🇳  Chức năng sẽ ra mắt trong bản chính thức\n\n" ..
                "🕐  %d ngày  %02d:%02d:%02d\n\n" ..
                "🇬🇧  Feature coming in official version\n\n" ..
                "🕐  %d days  %02d:%02d:%02d",
